...
def name_score(full: str, candidate: str) -> float:
    ...
    if nam:
        if nam in parts:
            ...
            # Имя должно совпадать целиком. Иначе «Олег» ошибочно совпадает
            # с отчеством «Олеговна» у другого врача с той же фамилией.
            score = min(score, 0.4)
...
def search_yandex(full_name: str, ref_photo: bytes | None = None) -> list[dict]:
    """Ищем только персональную карточку врача на Яндекс Картах."""
...
def pick_best(candidates: list[dict], *, min_confidence: float = 0.55) -> dict | None:
    ...
    ok = [
        ...
        and not _is_clinic_yandex_fallback(c)
    ]
    if not ok:
